...
const chatApiClient = axios.create({
  baseURL: import.meta.env.VITE_SOCKET_URI,
  withCredentials: true,
  timeout: 120000,
});
...
const loginUser = (data: { identifier: string; password: string }) => {
  console.log(data);
  return axios.post(`${import.meta.env.VITE_SERVER_URI}/auth/local`, data);
};
...
const getUserChats = () => {
  return chatApiClient.get("/chat-app/chats");
};

const createUserChat = (receiverId: string) => {
  return chatApiClient.post(`/chat-app/chats/${receiverId}`);
};
...
const getChatMessages = (chatId: string) => {
  console.log(chatId)
  return chatApiClient.get(`/chat-app/messages/${chatId}`);
};
...
const sendMessage = (chatId: string, content: string, attachments: File[]) => {
  ...
  return chatApiClient.post(`/chat-app/messages/${chatId}`, {content:content});
};
